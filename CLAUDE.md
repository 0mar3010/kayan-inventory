...
- **Excel ingestion (Phase 1, now built):** `/sync` upload page + `app/api/sync/upload/route.ts` + `lib/sync/*` (`parse-excel.ts` reads the local `.xls`/`.xlsx` via SheetJS with dynamic column detection on the Arabic headers الرصيد/أسم الصنف/رقم الموديل; `derive-brand.ts` maps Arabic name keywords → the catalog's exact brand strings since the sheet has no brand column; `ingest.service.ts` runs each row through `matchRow()`, creates a `SyncRun`+`SyncRunItem` per row, **auto-applies only exact (confidence 1.0) matches** — writes `InventorySnapshot` + `confirmMapping` in a tx — and routes everything fuzzy/ambiguous to the review queue, unmatched left untouched). Sample `localStock.xls` = 537 rows, 288 brand-derivable.
